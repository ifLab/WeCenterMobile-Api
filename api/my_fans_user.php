...
//查总数
$total_rows = $db->select("COUNT(*) AS total_rows")->from(TABLE_PREFIX.'user_follow')->where(array('friend_uid'=>$get['uid']))->get()->row_array();
...
$rows = $db->select("fans_uid")->from(TABLE_PREFIX.'user_follow')->where(array('friend_uid'=>$get['uid']))->order_by('add_time','DESC')->limit($per_page,$offset)->get()->result_array();
$str = array();
foreach( $rows as $k => $v ){
	$str[] = $v['fans_uid'];
}
