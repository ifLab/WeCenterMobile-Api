...
//查总数
$total_rows = $db->select("COUNT(*) AS total_rows")->from(TABLE_PREFIX.'article')->where(array('uid'=>$get['uid']))->get()->row_array();
...
$rows = $db->select("id,title,message,add_time")->from(TABLE_PREFIX.'article')->where(array('uid'=>$get['uid']))->order_by('add_time','DESC')->limit($per_page,$offset)->get()->result_array();
